...
The game window will display a 3x3 grid and a "New Game" button.
Click any empty cell to place an 'X'.
The computer will respond by placing an 'O' in a strategic or random position.
The game ends when a player wins (three identical symbols in a row, column, or diagonal) or when the board is full (a draw).
Click "New Game" to reset the board and play again.
